...
function procmux() {
  ...
  var subscribers = new _map2.default();
  var state = {};
  var reducers = {};
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            ...
            case 7:
              ...
              return _context2.abrupt('return', forks.set(forkID, child));
  ...
  if (!orphan) {
    process.on('message', function (action) {
      ...
      if (subscribers.has(type)) process.send();
      switch (type) {
        case DISPATCH:
          process.send();
      }
  ...
  return { fork: fork, dispatchParent: dispatchParent, dispatchFork: dispatchFork, dispatch: dispatch, subscribe: subscribe, getState: getState, reducer: reducer, exit: exit, kill: kill, orphan: orphan };
}